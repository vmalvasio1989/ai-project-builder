...
function toggleTaskStatus(projectId, requirementId, taskId) {
...
function renderProjectDetail() {
    ...
    const requirementTitle = document.querySelector("#requirementTitle");
...
function handleAddTask(projectId, requirementId) {
    const taskTitle = document.querySelector(`#taskTitle-${requirementId}`);
...
function editTask(reqId, taskId) {
    const projectAux = getSelectedProject();
    //Si projectAux esta vacio
    if (!projectAux) {
        return;
...
function deleteRequirement(reqId) {
    if (confirm("¿Estás seguro de eliminar este requerimiento?")) {
        ...
        projectAux.requirements = projectAux.requirements.filter(projectReq => projectReq.id !== reqId);
        updateDetail();
    }
}
...
function addRequirement(projectId, title) {
    const projectAux = getProjectById(projectId);
    //Si projectAux esta vacio
    if (!projectAux) {
        return;
...
function addTask(projectId, requirementId, title) {
    const projectAux = getProjectById(projectId);
    //Si projectAux esta vacio
    if (!projectAux) {
        return;
...
function deleteTask(reqId, taskId) {
...
function getRequirementById(project, requirementId) {
    return project.requirements.find(requirement => requirement.id === requirementId);
}
function getTaskById(requirement, taskId) {
